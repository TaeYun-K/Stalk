...
const ConsultationRecordingsPage: React.FC = () => {
  ...
  const [analysisByRecordingId, setAnalysisByRecordingId] = useState<
    Record<number, VideoAnalysisResult | null>
  >({});
  const [analyzingByRecordingId, setAnalyzingByRecordingId] = useState<
  ...
  const handleAnalyzeVideo = async (videoUrl: string, recordingId: number) => {
    try {
      ...
      if (!token) {
        ...
        setAnalyzingByRecordingId((prev) => ({
          ...prev,
          [recordingId]: false,
        }));
        return;
  ...
  useEffect(() => {
    ...
    const checkExistingAnalyses = async (
      recordingsToCheck: VideoRecording[]
    ) => {
      const token = AuthService.getAccessToken();
      if (!token) return;

      const results: Record<number, VideoAnalysisResult | null> = {};

      const promises = recordingsToCheck.map(async (recording) => {
        try {
          const response = await fetch(
            `/api/ai/analysis-result/${recording.id}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          if (response.ok) {
            ...
            results[recording.id] = null;
          }
        } catch (error) {
          console.error(
            `Error fetching analysis for recording ${recording.id}:`,
            error
          );
          results[recording.id] = null;
        }
      ...
      if (isMounted) {
        setAnalysisByRecordingId((prev) => ({ ...prev, ...results }));
      }
    };
    ...
    const load = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setRecordings(list);
          if (list.length > 0) {
            await checkExistingAnalyses(list);
          }
        }
      } catch (e: any) {
  ...
  const contentRightPaddingPx = useMemo(() => {
    const collapsedPx = isMobile ? 56 : 64;
    const expandedPanelPx = isMobile ? 256 : 320;
    const basePagePaddingPx = 24;
    return (
      (isSidebarExpanded ? collapsedPx + expandedPanelPx : collapsedPx) +
      basePagePaddingPx
    );
  ...
  return (
    <div className="min-h-screen bg-white">
      <div
        className="max-w-7xl mx-auto px-6 py-8 pt-28"
        style={{ paddingRight: contentRightPaddingPx }}
      >
                  ...
                  <div className="w-full max-w-3xl mx-auto space-y-4">
                    ...
                    <div className="grid grid-rows-2 border border-red-200 bg-red-50 rounded-lg p-4 mb-6">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
                        ...
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <span>
                            {new Date(recording.endTime).toLocaleString(
                              "ko-KR"
                            )}
                          </span>
                        </div>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                      <div className="flex items-start">
                        ...
                        <div className="w-full">
                          <h3 className="font-semibold text-left text-blue-800 mb-2">
                            Stalk AI가 상담 영상을 자동으로 요약해드립니다!
                          </h3>
                          <div className="space-y-1">
                            <p className="text-blue-700 text-sm text-left">
                              · 상담내용을 전문가가 직접 분석 작성한 상담일지에
                              대한 신뢰도와 정확성을 책임집니다.
                            </p>
                            <p className="text-blue-700 text-sm text-left">
                              · 영상 요약은 약 30초 ~ 1분 가량 소요될 수
                              있습니다.
                            </p>
                          </div>
                          <hr className="w-full my-4 border-blue-200" />
                          <p className="text-left text-sm text-gray-500">
                            ※ AI 요약 상담 일지는 상담 녹화 영상이 있을 때
                            제공됩니다.
                          </p>
                        </div>
                      </div>
                    ...
                    {recording.url && (
                      <div>
                        <button
                          onClick={() =>
                            handleAnalyzeVideo(recording.url, recording.id)
                          }
                    ...
                    {hasAnalysis && (
                      <div className="mt-2 bg-white rounded-lg shadow-lg p-6 border border-gray-200">
                        ...
                        {(() => {
                              ...
                              <div className="space-y-6">
                                {parsed.lecture_content &&
                                  parsed.lecture_content.length > 0 && (
                                    <div>
                                      <h4 className="text-lg font-semibold text-left text-gray-900 mb-3">
                                        📚 강의 내용
                                      </h4>
                                      <div className="space-y-8 rounded-lg p-6 border border-gray-200">
                                        {parsed.lecture_content.map(
                                          (
                                            item: {
                                              topic: string;
                                              details: string;
                                            ...
                                            >
                                              <h5 className="font-medium font-semibold text-blue-600 mb-2 text-left">
                                                {item.topic}
                                              </h5>
                                              <p className="pl-5 text-gray-700 leading-relaxed text-left">
                                                {item.details}
                                              </p>
                                            </div>
                                          )
                                ...
                                {parsed.key_takeaways && (
                                  <div>
                                    <h4 className="text-left text-lg font-semibold text-gray-900 mb-3 mt-8">
                                      🎯 핵심 요약
                                    </h4>
                                    <div className="flex flex-col bg-white rounded-lg p-4 border border-gray-200 space-y-3">
                                      <div className="flex flex-col items-start mb-4">
                                        <h5 className="font-medium text-gray-900 mb-2 font-semibold text-left">
                                          📌 주요주제
                                        </h5>
                                        <span className="text-gray-700 font-normal text-left pl-6">
                                          {parsed.key_takeaways.main_subject}
                                        </span>
                                      </div>
                                      {parsed.key_takeaways.core_concepts &&
                                        parsed.key_takeaways.core_concepts
                                          .length > 0 && (
                                          <div>
                                            <h5 className="font-semibold text-gray-900 mb-2 text-left">
                                              ⭐ 핵심개념
                                            </h5>
                                            <div className="flex flex-col items-start bg-yellow-50 rounded-lg py-4 px-7">
                                              <ol className="list-decimal list-inside space-y-1 text-gray-800 text-left space-y-2">
                                                {parsed.key_takeaways.core_concepts.map(
                                                  (
                                                    concept: string,
                                                    cidx: number
                                                  ) => (
                                      ...
                                      {parsed.key_takeaways
                                        ...
                                        <div className="flex flex-col items-start pt-4">
                                          ...
                                          <p className="pl-6 text-gray-700 leading-relaxed text-left">
                                            {
                                              parsed.key_takeaways
                                                .conclusion_and_strategy
                                            }
                          ...
                          // 2. JSON 파싱에 실패했지만 텍스트가 있는 경우 (금융 외 영상)
                          else if (summaryText) {
                            return (
                              <div className="text-gray-700 text-left whitespace-pre-wrap p-4 bg-gray-50 rounded-md">
                                {summaryText}
                              </div>
                            );
                          }
