...
const AdvisorsIntroductionUpdatePage: React.FC = () => {
  ...
  const [initialCareerMap, setInitialCareerMap] = useState<
    Record<
      number,
      {
        startDate: string;
    ...
    >
  >({});
  const [deletedCareerIds, setDeletedCareerIds] = useState<Set<number>>(
    new Set()
  );
  ...
  const handleSubmitAll = async () => {
    try {
      ...
      const deleteEntries: CareerPayload[] = Array.from(deletedCareerIds).map(
        (apiId) => {
