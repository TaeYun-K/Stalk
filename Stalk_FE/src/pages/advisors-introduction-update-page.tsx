...
// 전문가 상세 응답의 필요한 필드만 정의
interface AdvisorCareerApi {
  id: number;
  title: string;
  description: string;
  started_at: string;
...
const AdvisorsIntroductionUpdatePage: React.FC = () => {
  ...
  // 시간 설정 완료 상태
  const [isOperatingHoursComplete, setIsOperatingHoursComplete] = useState(false);
  ...
  // ISO(또는 파싱 가능한) 날짜 문자열을 YYYY.MM.DD로 변환
  const toYmdDot = (dateString: string) => {
  ...
  useEffect(() => {
    const fetchExpert = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error('전문가 정보를 불러오지 못했습니다.');
        }
        const data: { result?: AdvisorDetailApi } = await response.json();
        ...
        if (result.consultation_fee !== undefined && result.consultation_fee !== null) {
  ...
  const handleSubmitAll = async () => {
    try {
      ...
      const returnedId = (response && (response.id as string | number | undefined)) ?? undefined;
      ...
      const message = error instanceof Error ? error.message : '수정 중 오류가 발생했습니다. 다시 시도해주세요.';
          ...
          <div className="flex-1 space-y-12">
            ...
            <section className="space-y-8">
              ...
              <div className="space-y-4">
                <h3 className="text-left text-xl font-semibold text-black">프로필 사진 수정</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="w-full pl-10 text-left bg-gray-100 rounded-lg p-4 mb-6">
                  <ul className="text-left text-sm text-gray-700 space-y-3 py-2">
                    ...
                    <li>• 경력사항 입력 후 반드시 (+) 버튼을 눌러 추가해주세요.</li>
                  </ul>
                </div>
            ...
            <section className="space-y-8">
              ...
              {/* 상담 가능 거래 스타일 (단일 선택) */}
              <div className="space-y-2">
                <label className="block text-left text-lg font-semibold text-black">상담 가능 거래 스타일</label>
              ...
              <div className="space-y-2">
                <label className="block text-left text-lg font-semibold text-black">시간 당 상담료 설정
                  <span className="text-sm text-gray-500 ml-2 font-normal">(단위: 원)</span>
                </label>
                <input
                  type="text"
                  inputMode="numeric"
                ...
                />
                <p className="text-xs text-gray-500">현재는 표시만 합니다. 추후 API 연결 시 서버에 저장됩니다.</p>
              </div>

              {/* 상담 영업 시간 설정 */}
              <div>
                <h3 className="py-3 text-left text-lg font-semibold text-black">상담 영업 시간 설정</h3>  
                <AdvisorTimeTable onOperatingHoursChange={setIsOperatingHoursComplete} />
              </div>
            </section>
          </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className={`w-2 h-2 rounded-full ${isPreferredTradeStyleComplete() ? 'bg-blue-500' : 'bg-gray-300'}`}></div>
                      <span>상담 가능 거래 스타일</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className={`w-2 h-2 rounded-full ${isConsultationFeeComplete() ? 'bg-blue-500' : 'bg-gray-300'}`}></div>
                      <span>시간당 상담료 설정</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className={`w-2 h-2 rounded-full ${checkOperatingHoursComplete() ? 'bg-blue-500' : 'bg-gray-300'}`}></div>
...
export default AdvisorsIntroductionUpdatePage;