...
const SignupPage = () => {
  ...
  const formatTime = (seconds: number) => {
    ...
    return `${mins.toString().padStart(2, "0")}분 ${secs
      .toString()
      .padStart(2, "0")}초`;
  };
  ...
  const handleUserIdCheck = async () => {
    ...
    try {
      const response = await fetch(
        `/api/auth/duplicate-check?${encodeURIComponent(
          "id|nickname"
        )}=id&value=${formData.userId}`
      );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!passwordPattern.test(formData.password)) {
      setErrorMessage(
        "비밀번호는 8~20자이며, 숫자·대문자·소문자·특수문자를 모두 포함해야 합니다."
      );
      return;
    }
    ...
    if (userType === "expert") {
      if (
        !formData.qualification.certificateName ||
        !formData.qualification.certificateFileSn ||
        !formData.qualification.birth ||
        !formData.qualification.certificateFileNumber
      ) {
    ...
    try {
      ...
      if (userType === "expert") {
        ...
        formDataToSend.append(
          "contact",
          formData.contact.replace(/[^0-9]/g, "")
        );
        formDataToSend.append(
          "email",
      ...
      if (result.userId || result.success) {
              ...
              if (loginResult.accessToken) {
                ...
                try {
                  ...
                  if (qualificationResponse.ok) {
                    alert(
                      "회원가입이 완료되었습니다! 자동 로그인되었고 자격증 인증요청이 접수되었습니다."
                    );
                  } else {
                    alert(
                      "회원가입이 완료되었습니다! 자동 로그인되었습니다. (자격증 인증요청은 별도로 진행해주세요)"
                    );
                  }
                ...
                alert(
                  "회원가입이 완료되었습니다! 로그인 후 자격증 인증요청을 진행해주세요."
                );
              }
            } else {
              alert(
        ...
        navigate("/signup-complete", {
          state: { name: formData.name, userType },
        });
      } else {
  ...
  const isAllTermsAgreed =
    formData.privacyAgreement && formData.thirdPartyAgreement;
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {userType === "expert" && (
              <div className="border-b border-gray-200 pb-6">
                <h3 className="text-lg font-semibold text-gray-900 text-left mb-4">
                  프로필 사진
                </h3>
                <div className="flex items-start space-x-6">
                  <div className="aspect-[3/4] w-1/6 max-w-[250px] bg-gray-100 rounded-lg flex items-center justify-center border border-gray-300">
                    {formData.profilePhoto ? (
                      <img
                        ...
                        alt="Profile"
                        className="w-120 h-160 rounded-lg object-cover"
                      />
                    ) : (
                      <svg
                  ...
                  <div className="w-5/6 flex-1 flex flex-col gap-4">
                    ...
                    <div className="text-m text-gray-500 space-y-1 text-left flex flex-col gap-2">
                      ...
                      <p>
                        • 파일 형식은 JPGE(.jpg, .jpeg) 또는 PNG(.png)만
                        지원합니다.
                      </p>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            ...
            <div className="p-6 text-left">
              <p className="text-gray-700 mb-6">
                Stalk는 회원가입 및 서비스 제공을 위해 아래와 같이 개인정보를
                수집·이용합니다.
              </p>
      ...
      {showThirdPartyModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            ...
            <div className="p-6">
              <p className="text-gray-700 mb-6">
                Stalk는 서비스 제공을 위해 아래와 같이 개인정보를 제3자에게
                제공할 수 있습니다.
              </p>
