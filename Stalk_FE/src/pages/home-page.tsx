...
const HomePage: React.FC = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.isSuccess && data.result.content) {
          // 현재 날짜보다 미래의 예약만 필터링
          const now = new Date();
          const futureReservations = data.result.content.filter(
            (reservation) => {
              const reservationDateTime = new Date(
                `${reservation.consultationDate}T${reservation.consultationTime}`
              );
          ...
          futureReservations.sort((a, b) => {
            const dateA = new Date(
              `${a.consultationDate}T${a.consultationTime}`
            );
            const dateB = new Date(
              `${b.consultationDate}T${b.consultationTime}`
            );
  ...
  const fetchExperts = useCallback(async () => {
    try {
      ...
      if (data.isSuccess) {
        ...
        const sortedExperts = [...data.result.content].sort(
          (a, b) =>
  ...
  useEffect(() => {
    const fetchRecentPosts = async () => {
      try {
        ...
        setPostsError(null);
        const res = await AuthService.publicRequest(
          "/api/community/posts?category=ALL&pageNo=1&pageSize=20",
          { method: "GET" }
        );
  ...
  const filteredExperts = experts.filter(
    (expert) =>
      selectedInvestmentStyle === "ALL" ||
      expert.preferredStyle === selectedInvestmentStyle
  );
        ...
        {isLoggedIn && (
          <section className="w-full mb-16 bg-gray-100 pt-10 pb-10 px-36">
            <div className="relative flex items-center gap-3 mb-8">
              <h2 className="text-2xl hoverxl font-bold text-secondary-900 relative z-10">
                예정된 상담 내역
              </h2>
              <button
            ...
            <div className="flex flex-row items-center gap-4 overflow-hidden">
              ...
              {reservations.length > pageSize && (
                <button
                  ...
                  className={`px-3 py-6 text-xl font-bold rounded-lg transition-colors ${
                    currentIndex === 0
                ...
                {loading ? (
                  ...
                  reservations
                    ...
                    .map((reservation) => {
                      ...
                      const formattedDate = `${reservation.consultationDate
                        .split("-")
                        .join(". ")}(${dayOfWeek})`;
                      const formattedTime =
                        reservation.consultationTime.substring(0, 5);
                          ...
                          <div className="flex items-center justify-start">
                            <div className="flex flex-col justify-start gap-4">
                              <div className="text-blue-500 flex justify-between items-center gap-3">
                                ...
                                <div className="text-xl text-left font-bold text-gray-900">
                                  {formattedTime}
                                </div>
                              </div>
                              <div className="flex flex-row justify-center items-end gap-2">
                                <p className="text-gray-700 font-semibold text-md font-light">
                ...
                <button
                  ...
                  onClick={() =>
                    setCurrentIndex((prev) =>
                      prev + pageSize < reservations.length
                        ? prev + pageSize
                        : prev
                    )
                  }
                  ...
                  className={`px-3 py-6 text-xl font-bold rounded-lg transition-colors ${
                    currentIndex + pageSize >= reservations.length
                      ? "text-gray-300 cursor-not-allowed"
                      : "text-gray-700 hover:text-blue-500"
                  }`}
                >
        ...
        <div className="relative pt-10 pr-20">
          <main className="px-20 sm:px-6 lg:px-36 py-8 pt-1">
            ...
            <section className="max-w-7xl mx-auto mb-16">
              <div>
                <div className="relative flex items-center gap-3 mb-8">
                  <p className="text-2xl hoverxl font-bold text-secondary-900 relative z-10">
                    최근 상담글
                  </p>
                  <button
                    ...
                    {postsLoading ? (
                      ...
                      posts.slice(0, 2).map((p, idx, arr) => (
                        <div
                          key={p.postId}
                          onClick={() =>
                            navigate(`/investment-knowledge-detail/${p.postId}`)
                          }
                          className={`flex flex-col justify-start gap-3 ${
                            idx < arr.length - 1
                              ? "border-b border-gray-200"
                              : ""
                          } py-6 cursor-pointer hover:bg-gray-50`}
                        >
                          <span className="text-left text-blue-600 text-md font-semibold">
                            #{getCategoryKorean(p.category)}
                          </span>
            ...
            <section className="max-w-7xl mx-auto mb-16">
              <div className="relative flex items-center gap-3 mb-8">
                <p
                  className="text-2xl hoverxl font-bold text-secondary-900 relative z-10 cursor-pointer hover:text-orange-500"
                  onClick={() => navigate("/advisors-list")}
                  title="투자 전문가 목록으로 이동"
                >
                  새로 함께하는 컨설턴트
                </p>
                <div className="text-white text-sm font-semibold bg-gradient-to-r from-orange-400 to-pink-500 px-2 rounded-md">
                  New
                </div>
                <button
                  onClick={() => navigate("/advisors-list")}
              ...
              <div className="flex flex-row items-center gap-4 overflow-hidden">
                ...
                {filteredExperts.length > pageSize && (
                  <button
                    aria-label="이전 전문가들"
                    onClick={() =>
                      setExpertsIndex((prev) => Math.max(0, prev - pageSize))
                    }
                      ...
                      .map((expert) => {
                        ...
                        return (
                          <div
                            ...
                            onClick={() =>
                              navigate(`/advisors-detail/${expert.id}`)
                            }
                          >
                            <div
                              className={`text-left text-sm font-semibold ${cls.headerText} ${cls.headerBg} w-full rounded-t-lg px-5 py-2`}
                            >
                            ...
                            <div className="flex flex-col items-start justify-start">
                              <div className="flex flex-col justify-start gap-2 flex-1 px-5 py-4">
                                <div className="flex flex-row justify-start items-end gap-2">
                                  ...
                                  <p
                                    className={`text-sm font-medium ${cls.accentText}`}
                                  >
                                    컨설턴트
                                  </p>
                                ...
                                <p className="truncate text-left text-sm font-light w-full">
                                  {expert.shortIntro}
                                </p>
                              </div>
                              <div
                                className={`flex justify-between items-center w-full bg-gray-100 text-xs ${cls.accentText} font-semibold px-5 py-2`}
                              >
                                <p>시간당 상담료</p>
                ...
                {filteredExperts.length > pageSize && (
                  <button
                    aria-label="다음 전문가들"
                    onClick={() =>
                      setExpertsIndex((prev) =>
                        prev + pageSize < filteredExperts.length
                          ? prev + pageSize
                          : prev
                      )
                    }
                    ...
                    className={`px-3 py-6 text-xl font-bold rounded-lg transition-colors ${
                      expertsIndex + pageSize >= filteredExperts.length
                        ? "text-gray-300 cursor-not-allowed"
                        : "text-gray-700 hover:text-orange-500"
                    }`}
                  >
