...
const indicatorExplanations = {
  ...
  macd: {
    ...
    params:
      "단기(12), 장기(26), 시그널(9)이 기본값 (최소 26개 데이터 포인트 필요)",
  },
  stochastic: {
    title: "스토캐스틱",
    description:
      "일정 기간 중 현재 가격의 상대적 위치를 나타내는 모멘텀 지표입니다.",
    usage:
      "80 이상: 과매수, 20 이하: 과매도. %K와 %D선의 교차로 매매 신호 포착",
    params:
      "%K 기간, %D 기간 (smoothing) - 일반적으로 14일 사용 (최소 14개 데이터 포인트 필요)",
  },
};
...
function parseOvData(raw: string): any {
  ...
  try {
    ...
    if (obj?.clientData && typeof obj.clientData === "string") {
      try {
        return JSON.parse(obj.clientData);
      } catch {}
    }
    if (obj?.serverData && typeof obj.serverData === "string") {
      try {
        return JSON.parse(obj.serverData);
      } catch {}
    }
  ...
  if (mClient) {
    try {
      return JSON.parse(decodeURIComponent(mClient[1]));
    } catch {}
  }
  const mServer = /serverData=([^,&]+)/.exec(raw);
  if (mServer) {
    try {
      return JSON.parse(decodeURIComponent(mServer[1]));
    } catch {}
  }
...
const VideoConsultationPage: React.FC = () => {
  ...
  const [subscriberStatusMap, setSubscriberStatusMap] = useState<
    Record<string, { audio: boolean; video: boolean }>
  >({});
  const [hasUnreadMessages, setHasUnreadMessages] = useState(false);
  const [isInSession] = useState(true);
  const ovTokenRef = useRef<string | null>(
    ovToken ?? sessionStorage.getItem("ovToken")
  );
  const consultationIdRef = useRef<string | null>(
    consultationId ?? sessionStorage.getItem("consultationId")
  );
  ...
  const cleanupScreenShare = async () => {
    ...
    try {
      ...
      if (screenSession) {
        try {
          await screenSession.disconnect();
        } catch (e) {
          console.warn(
            "[cleanupScreenShare] screenSession.disconnect failed:",
            e
          );
        }
      }
    ...
    try {
      ...
      if (ovToken) {
        ...
        session.on("streamCreated", (event) => {
          ...
          const isScreen =
            event.stream.typeOfVideo === "SCREEN" || meta?.kind === "screen";
          ...
          const fromMySecondConn =
            myScreenConnectionIdRef.current &&
            myScreenConnectionIdRef.current ===
              event.stream.connection.connectionId;
          ...
          if (
            isRecordingScreen ||
            (isScreen && (mine || fromThisConnection || fromMySecondConn))
          ) {
            console.log(
              "[OV] skip subscribe for recording-only screen stream:",
              {
        ...
        session.on("streamPropertyChanged", (event) => {
          ...
          setSubscriberStatusMap((prev) => ({
            ...
            [connectionId]: {
              ...prev[connectionId],
              [event.changedProperty === "audioActive" ? "audio" : "video"]:
                event.newValue,
            },
          }));
        });
        ...
        // 세션에 연결
        console.log(
          "Connecting to session with token:",
          ovToken.substring(0, 20) + "..."
        );
  ...
  const attachLocalVideo = (publisher: Publisher) => {
    ...
    if (videoElement && publisher.stream) {
      ...
      console.warn(
        "Local video element not found or publisher stream not ready"
      );
    }
  };
  ...
  const attachSubscriberVideo = (subscriber: Subscriber, index: number) => {
    ...
    if (videoElement.srcObject) {
      console.log(
        `Video element subscriber-video-${index} already has a stream`
      );
      return;
    }
  ...
  const handlePeriodChange = (period: number) => {
    ...
    setCurrentChart((prev) =>
      prev ? { ...prev, period: String(period) } : info
    );
  ...
  useEffect(() => {
    ...
    return () => {
      session.off("signal:chart:sync_request", onSyncReq);
    };
  ...
  useEffect(() => {
    ...
    return () => {
      session.off("signal:chart:sync_state", onSyncState);
    };
  }, [session]);
    ...
    try {
      ...
      if (!isMyScreenActive() && !hasAnyScreen(session)) {
        ...
        const tokenRes = await axios.post(
          `/api/recordings/sessions/${encodeURIComponent(
            ovSessionId
          )}/connections`,
          {},
          {
        ...
        const screenToken =
          tokenRes?.data?.result?.token ?? tokenRes?.data?.data?.token;
        ...
        sess2.on("streamCreated", (event) => {
          console.log(
            "[recording] Screen session received stream, but not subscribing:",
        ...
        myScreenConnectionIdRef.current =
          sess2.connection?.connectionId || null;
        console.log(
          "[recording] Screen connection ID set:",
          myScreenConnectionIdRef.current
        );
        ...
        console.log(
          "[recording] screen exists → skip creating second connection"
        );
      }

      // 2) 녹화 시작 - 한 번만 호출
      const recRes = await axios.post(
        `/api/recordings/start/${encodeURIComponent(
          ovSessionId
        )}?consultationId=${encodeURIComponent(String(consultationId))}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );

      const recId =
        (recRes?.data?.data?.recordingId as string) ||
        (recRes?.data?.result?.recordingId as string) ||
        ovSessionId;
  ...
  const handleStopRecording = async () => {
    ...
    try {
      ...
      const stopUrl = consultationId
        ? `/api/recordings/stop/${encodeURIComponent(
            recordingId
          )}?consultationId=${encodeURIComponent(String(consultationId))}`
  ...
  const leaveSession = async (): Promise<void> => {
    ...
    try {
      ...
      await axios.post(
        `/api/consultations/${id}/session/close`,
        {},
        {
          headers: {
    ...
    if (ovToken && consultationId) {
      const initializeConsultation = async () => {
        try {
          ...
          console.log(
            "User info fetch completed, OpenVidu initialization will be handled separately"
          );
        } catch (error) {
  ...
  useEffect(() => {
    if (
      publisher &&
      isVideoEnabled &&
      (!showStockChart || showParticipantFaces || showParticipants)
    ) {
      setTimeout(() => {
        attachLocalVideo(publisher);
      }, 100);
      ...
      <div
        className={`${
          showStockChart
            ...
            : "bg-gray-800"
        } px-6 py-3 flex items-center justify-between border-b border-gray-700 transition-all duration-300 relative overflow-visible`}
      >
          ...
          {showStockChart && (
            <>
              ...
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-gray-800/40 backdrop-blur-md border border-gray-700/30">
                <ChartControls
                  ...
                  onPeriodChange={(period) =>
                    handlePeriodChange(parseInt(period))
                  }
                  onChartTypeChange={() => {}}
                  darkMode={true}
                />
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    title={
                      dataPointCount < 14
                        ? `RSI requires 14+ data points (current: ${dataPointCount})`
                        : ""
                    }
                  >
                    RSI
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    title={
                      dataPointCount < 26
                        ? `MACD requires 26+ data points (current: ${dataPointCount})`
                        : ""
                    }
                  >
                    MACD
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    title={
                      dataPointCount < 14
                        ? `Stochastic requires 14+ data points (current: ${dataPointCount})`
                        : ""
                    }
                  >
                    스토캐스틱
              ...
              <div className="flex items-center">
                <button
                  onClick={() => {
                    ...
                    console.log(
                      "Consultation: Toggling drawing mode from",
                      isDrawingMode,
                      "to",
                      newDrawingMode
                    );
              ...
              <div className="ml-auto w-55 relative z-[1000] [&_input]:!py-0.5 [&_input]:!text-xs [&_input]:!px-2 [&_.mb-5]:!mb-0 [&_input]:!h-7 [&_.relative]:!mb-0 [&_.px-4.py-3]:!px-2 [&_.px-4.py-3]:!py-2">
                <StockSearch onStockSelect={setSelectedStock} darkMode={true} />
              </div>
            </>
          )}
              ...
              {subscribers.length > 0 ? (
                subscribers
                  ...
                  .map((subscriber, index) => {
                    ...
                    const mediaStatus = subscriberStatusMap[connectionId] || {
                      audio: false,
                      video: true,
                    };
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-lg text-gray-300">
                        {getRoleDisplayName(
                          userInfo?.role === "ADVISOR" ? "USER" : "ADVISOR"
                        )}{" "}
                        대기 중
                      </p>
                    </div>
              ...
              <div className="bg-gray-800 rounded-lg overflow-hidden relative">
                ...
                <div className="absolute bottom-3 left-3 bg-black/60 backdrop-blur-sm px-2.5 py-1 rounded-lg">
                  <span className="text-xs font-medium">
                    ...
                    {getRoleDisplayName(
                      ((userInfo?.role || "USER") as "ADVISOR") || "USER"
                    )}
                    )
                  </span>
                </div>
                  ...
                  {subscribers.map((subscriber) => {
                    ...
                    return (
                      <div
                        ...
                        className="w-full aspect-video bg-gray-700 rounded-lg overflow-hidden relative shadow-lg hover:shadow-xl transition-shadow duration-200"
                      >
                        <video
                          ref={(videoElement) => {
                            if (videoElement && subscriber.stream) {
                  ...
                  {subscribers.map((subscriber, index) => (
                    <div key={index} className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-400">
                          {getRoleDisplayName(getParticipantRole(subscriber))}
                        </p>
                      </div>
                      <div className="flex space-x-1">
                ...
                <h4 className="font-bold text-sm mb-1 text-blue-400">
                  {
                    indicatorExplanations[
                      hoveredIndicator as keyof typeof indicatorExplanations
                    ]?.title
                  }
                </h4>
