...
class WatchlistService {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        const mockResults: WatchlistItem[] = [
          ...
          (item) => item.name.includes(query) || item.code.includes(query)
        );
        resolve(mockResults);
      }, 800);
    });
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({
          ...
          pbr: 1.1,
          description:
            "대한민국의 대표 기업으로 반도체, 스마트폰 등을 제조하는 글로벌 기술기업입니다.",
        });
      }, 1000);
    });
  }
  ...
  static async updateStockPrices(
    ...
    [code: string]: { price: number; changeAmount: number; changeRate: number };
  }> {
      ...
      setTimeout(() => {
        ...
        codes.forEach((code) => {
          updates[code] = {
            ...
            changeRate: parseFloat(((Math.random() - 0.5) * 5).toFixed(2)),
          };
        });
        resolve(updates);
      }, 500);
