...
class AuthService {
  ...
  static async authenticatedRequest(
    ...
    console.log(
      "authenticatedRequest - token value:",
  ...
  static async publicRequest(
    ...
    const config: RequestInit = {
      ...
      headers: {
        ...options.headers,
        // body가 JSON이면 자동으로 Content-Type 세팅
        ...(!(options.body instanceof FormData) && {
          "Content-Type": "application/json",
        }),
      },
  ...
  static async signup(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || `회원가입 실패: ${response.status}`
        );
      }
  ...
  // 전문가 회원가입
  static async advisorSignup(
    data: AdvisorSignupRequest
  ): Promise<{ success: boolean; message: string }> {
    try {
      // 입력값 검증
      if (
        ...
        !data.certificateFileSn ||
        !data.birth ||
        !data.certificateFileNumber ||
        !data.profileImage
      ) {
        throw new Error("모든 필수 항목을 입력해주세요.");
      }
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || `전문가 회원가입 실패: ${response.status}`
        );
      }
